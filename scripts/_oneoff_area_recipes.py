...
def replace_marked(text: str, start: str, end: str, block: str) -> str:
    ...
    raise RuntimeError(f'missing marked block: {start}')
...
def ensure_registry_anchor(text: str, heading: str, marker: str, anchor: str) -> str:
    tag = f'<a id="{anchor}"></a>'
    if tag in text:
        return text
    pattern = re.compile(rf'(?m)^(### {re.escape(heading)}\n\n<!-- {re.escape(marker)}:START -->)')
    text, count = pattern.subn(tag + r'\n\1', text, count=1)
    if count != 1:
        raise RuntimeError(f'could not place registry anchor {anchor}')
    return text


ZH_ONBOARDING = textwrap.dedent(r'''
<!-- ONBOARDING:START -->
<div align="center">
...
EN_ONBOARDING = textwrap.dedent(r'''
<!-- ONBOARDING:START -->
<div align="center">
...
def update_readmes() -> None:
    for filename, onboarding, recipes in (
        ('README.md', ZH_ONBOARDING, ZH_RECIPES),
        ('README.en.md', EN_ONBOARDING, EN_RECIPES),
    ):
        path = ROOT / filename
        text = path.read_text(encoding='utf-8')
        text = replace_marked(text, '<!-- ONBOARDING:START -->', '<!-- ONBOARDING:END -->', onboarding)
        if '<!-- EVALUATION-RECIPES:START -->' in text:
            text = replace_marked(text, '<!-- EVALUATION-RECIPES:START -->', '<!-- EVALUATION-RECIPES:END -->', recipes)
        else:
            frontier = '<a id="frontier-signals"></a>'
            if frontier not in text:
                raise RuntimeError(f'{filename}: missing frontier anchor')
            text = text.replace(frontier, recipes + '\n\n' + frontier, 1)
        text = ensure_registry_anchor(text, 'Agent Memory', 'TABLE-FIRST:AREA:agent-memory', 'registry-memory')
        text = ensure_registry_anchor(text, 'RAG / Agentic Retrieval', 'TABLE-FIRST:AREA:rag', 'registry-rag')
        text = ensure_registry_anchor(text, 'Data Agents', 'TABLE-FIRST:AREA:data-agent', 'registry-data')
        path.write_text(text, encoding='utf-8')


def update_contracts() -> None:
    path = ROOT / 'CURATION.md'
    text = path.read_text(encoding='utf-8')
    old = ('README starts with a compact **onboarding layer** for project identity and reader routing, followed by the '
           '**signal-first, table-first research surface**. The onboarding layer is navigation rather than research synthesis. '
           '“Time-first” means that the newest research is easiest to scan; it does **not** mean that Radar-maintenance '
           'timestamps replace the research chronology or that one card is rendered per paper. Keep onboarding stable and short: '
           'title, one-sentence value proposition, language switch, lightweight status badges, and a Start Here router to canonical anchors.')
    new = ('README starts with a compact **onboarding layer** that routes readers by research area, followed by a small '
           '**Evaluation Recipes** decision-support layer, then the **signal-first, table-first research surface**. Onboarding '
           'and recipes are navigation/selection aids rather than research-trend synthesis. “Time-first” means that the newest '
           'research is easiest to scan; it does **not** mean that Radar-maintenance timestamps replace the research chronology '
           'or that one card is rendered per paper. Keep onboarding stable and short: title, one-sentence value proposition, '
           'language switch, lightweight status badges, and three area routes to each area’s map, recipes, and complete registry.')
    text = replace_once(text, old, new, 'CURATION onboarding contract')
    bullet = ('- **Evaluation recipes:** maintain a compact decision table for each area that maps common research claims to one '
              'Core benchmark, complementary coverage, and one explicit remaining inference gap. Keep 3–5 recipes per area. '
              'Recipes are curated starting points, not rankings, exhaustive experimental suites, or substitutes for reading '
              'protocol/confounder metadata.\n')
    if bullet not in text:
        text = text.replace('- **Recent release timeline:**', bullet + '- **Recent release timeline:**', 1)
    path.write_text(text, encoding='utf-8')

    path = ROOT / 'docs' / 'DAILY_WORKFLOW.md'
    text = path.read_text(encoding='utf-8')
    start = text.index('The public reader contract is **signal-first, then table-first**:')
    end = text.index('\n\nPer-item `<details>`', start)
    ...
    old_validation = ('Validation must additionally guard the signal-first and table-first contract: both README languages must begin '
                      'their research content with the three-row 30-day frontier-signal table, put the rolling release table immediately '
                      'after it, contain no public per-item deep-read or rolling 7-day/30-day synthesis surface, keep one defining chain '
                      'for each Benchmark Map area, keep one `What it tests / 考察内容` column rather than a parallel change column in the '
                      'main README tables, retain all three complete area tables in README, and never move those tables exclusively into `library/`.')
    new_validation = ('Validation must additionally guard the intent-first, signal-first, and table-first contract: both README languages '
                      'must expose the same three area routes, one bounded Evaluation Recipes block with 3–5 recipes per area, and stable '
                      'per-area registry anchors; the three-row 30-day frontier-signal table must remain the first research-trend synthesis '
                      'and the rolling release table must follow it. The main README must contain no public per-item deep-read or rolling '
                      '7-day/30-day synthesis surface, keep one defining chain for each Benchmark Map area, keep one `What it tests / 考察内容` '
                      'column rather than a parallel change column, retain all three complete area tables, and never move those tables '
                      'exclusively into `library/`.')
    text = replace_once(text, old_validation, new_validation, 'DAILY_WORKFLOW validation contract')
    path.write_text(text, encoding='utf-8')
    ...
    text = replace_once(text,
        ...
        'EDITORIAL navigation order')
    guidance = ('Evaluation Recipes are decision aids, not leaderboard recommendations. Phrase each row around a **research claim**, '
                'then name a Core benchmark, complementary coverage, and one inference boundary. Prefer 2–4 benchmarks per recipe. '
                'Do not rank benchmarks by citation count or recency, and do not imply that combining several system-level scores '
                'isolates a component mechanism.\n\n')
    marker = '## README scan contract\n\n'
    if guidance.strip() not in text:
        text = replace_once(text, marker, marker + guidance, 'EDITORIAL recipe guidance')
    path.write_text(text, encoding='utf-8')
...
def update_validator() -> None:
    ...
    text = path.read_text(encoding='utf-8')
    old = '''        release = text.find('<a id="release-timeline"></a>')
...
'''
    new = '''        onboarding_start = text.find("<!-- ONBOARDING:START -->")
        ...
        if release < 0 or field_map < 0 or release >= field_map:
            ...
            errors.append(f"{language}: per-item deep reads returned to the main README")
'''
    if 'recipe_start = text.find("<!-- EVALUATION-RECIPES:START -->")' not in text:
        text = replace_once(text, old, new, 'validate_public_readme routing block')
    if 'Chinese/English Evaluation Recipes benchmark-link drift' not in text:
        insertion = '''    def _recipe_external_links(value: str) -> list[str]:
        try:
...
'''
        target = '    try:\n        zh_recent = BENCHMARK_ID_RE.findall('
        if target not in text:
            raise RuntimeError('validator parity insertion point missing')
...
def update_tests() -> None:
    ...
    if __name__ == "__main__":
        unittest.main()
    ''').strip() + '\n'
    (ROOT / 'tests' / 'test_evaluation_recipes.py').write_text(test, encoding='utf-8')
