...
`data/locales/zh/benchmarks.json` contains the Chinese one-sentence descriptions;
English descriptions stay in `data/benchmarks.json`. Root READMEs and linked recipe
and result indexes are generated by `node scripts/render-readme.mjs`; `--check`
rejects drift. Names and numbers are never translated by regex replacement.

Long-form `benchmarks/<id>.md` and `<id>.en.md` remain authored sources, rendered
natively by GitHub. The retired website is not a public reader surface. Preserve
all load-bearing numbers, conditions, caveats and source references in both
languages. See [README-first publication](README_PUBLICATION.md).
