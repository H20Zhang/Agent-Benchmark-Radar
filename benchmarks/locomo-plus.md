...
> **发布时最佳结果（待核验）** · 基准记录日期：2026-02-11<br>
> 尚未核验原始发布版本中可定位到模型、指标和协议的最佳成绩。 [原始来源](https://aclanthology.org/2026.acl-long.1150/)<br>
...
适合研究‘相关历史没有被新问题直接提示’时的约束使用。真正有价值的对照不是又一次显式事实问答，而是同一潜在约束在直接提示和语义疏远提示下是否仍能生效。

### 一个具体任务长什么样

示意任务：用户早先表达某项长期限制，之后提出一个措辞完全不同的选择问题。合格回应应主动应用限制，而不是等用户重新提醒；但无关旧限制也不应无限扩展到所有问题。

### 最有判别力的实验

为相同历史制作直接提示、间接提示和明确不适用三种测试查询，固定回答模型。分别测约束召回、正确应用与过度应用；只报告一致性可能掩盖把过期或不相关约束强行带入新任务的错误。

### 建议搭配

[inmind](inmind.md) · [memtrapbench](memtrapbench.md)
